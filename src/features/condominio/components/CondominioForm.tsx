...
import { CondominioFormData, condominioFormSchema } from '@/entities/condominio/types';
...
export const CondominioForm = ({ onSave, onCancel, isSaving }: CondominioFormProps) => {
  ...
  return (
    <>
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSave)} className="space-y-6 py-4 max-h-[70vh] overflow-y-auto pr-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <FormField control={form.control} name="nome" render={({ field }) => ( <FormItem><FormLabel>Nome do Condomínio</FormLabel><FormControl><Input {...field} placeholder="Residencial das Flores" /></FormControl><FormMessage /></FormItem> )} />
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <FormField control={form.control} name="logradouro" render={({ field }) => ( <FormItem className="md:col-span-2"><FormLabel>Endereço</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem> )} />
            <FormField control={form.control} name="numero" render={({ field }) => ( <FormItem className="md:col-span-1"><FormLabel>Número</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem> )} />
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <FormField control={form.control} name="complemento" render={({ field }) => ( <FormItem><FormLabel>Complemento</FormLabel><FormControl><Input {...field} placeholder="Apto, Bloco, etc. (Opcional)" /></FormControl><FormMessage /></FormItem> )} />
